...
public class GameBoard implements com.whichoneiwonder.aiproj.fencemaster.Piece {
  ...
  /** Creates the top half of the game board From row 0 to row (Dimension - 1) * */
  ...
  /**
   * Creates the middle and longest row Starts and ends with 2 corner cells and the rest are Center
   * pieces *
   */
  ...
  /** Creates the top half of the game board From row Dimension to the last row * */
   ...
   *
   * <p>Prints the GameBoard *
   */
  protected void printBoard() {
    for (int row = 0; row < board.length; row++) {
      for (int col = 0; col < board[0].length; col++) {
        if (board[row][col] != null) System.out.print(board[row][col].getState() + "\t");
  ...
  protected void printBoard(PrintStream output) {
    ...
    for (int row = 0; row < board.length; row++) {
      ...
      for (int col = 0; col < board[0].length; col++) {
        if (board[row][col] != null) output.print(board[row][col].getState() + " ");
      }
      output.print("\n");
    }
  }
  ...
  /**
   * Method that sets the state of a cell in the GameBoard given the (row, col) positions and the
   * desired state
   *
   ...
   * Used for Debugging Purposes
   *
   * <p>Prints the static ArrayList edgeList consisting of all edge pieces *
   */
   ...
   * Used for Debugging Purposes
   *
   * <p>Prints the Cell position of all the connecting cells of a given cell
   *
  ...
  public boolean checkForBlank() {
    ...
    for (int row = 0; row < 2 * getDimension() - 1; row++) {
      for (int col = 0; col < 2 * getDimension() - 1; col++) {
        if (this.getCell(row, col) != null && this.getCell(row, col).getState() == 0) {
          return true;
        }
      }
    }
  ...
  /**
   * Handles the checking if a tripod is present in the GameBoard through DFS and a stack abstract
   * data structure implemented in an ArrayList
   *
   * @param colour = the type of state we are checking the tripod win state for that colour
  ...
  /**
   * Handles the checking if a loop is present in the GameBoard through DFS and a stack abstract
   * data structure implemented in an ArrayList
   *
   * @param colour = the type of state we are checking the tripod win state for that colour
        ...
        if (current != null
          ...
          for (int i = 0; i < 6; i++) {
            ...
            if (!(next.getState() == colour)) {
              ...
              if (!next.toString().equals("Center") || canReachEdge.contains(next)) {
  ...
  protected void addMove(Move mv) {
    ...
    for (Cell space : getCell(mv.Row, mv.Col).getAllLinks()) {
      if (space != null && space.state == EMPTY && !borderCellList.get(mv.P).contains(space)) {
  ...
  protected ArrayList<Cell> addToPriorityQueue(
      ArrayList<Cell> pq, Cell current, int edgeNum, int playerNum) {
